  ...
  return (
    <section className="w-full bg-white py-4 lg:py-[3rem] flex flex-col items-center justify-center gap-[2rem] lg:px-[8rem]  px-[1rem] sidebar">
      ...
      <div className="h-[30rem]  w-full  rounded-xl shadow-xl grid grid-cols-3 p-8 text-red-900 place-items-center justify-items-center items-center">
        ...
        <div className=""> <TbBrandJavascript className={` text-orange-700 ${isGlowing ? 'h-[5rem] w-[4rem]' : 'h-[3.5rem] w-[3.5rem]'}`} /> </div>
        <div className=""> <IoLogoReact className={` text-sky-800 ${isGlowing ? 'h-[5rem] w-[4rem]' : 'h-[3.5rem] w-[3.5rem]'}`} /></div>
        <div className="">  <TbBrandNextjs className={`text-black ${isGlowing ? 'h-[5rem] w-[4rem]' : 'h-[3.5rem] w-[3.5rem]'}`} /></div>
        <div className=""> <BiLogoTailwindCss className={` text-teal-500 ${isGlowing ? 'h-[5rem] w-[4rem]' : 'h-[3.5rem] w-[3.5rem]'}`} /> </div>
        <div className=""> <TbBrandTypescript className={` ${isGlowing ? 'h-[5rem] w-[4rem]' : 'h-[3.5rem] w-[3.5rem]'}`} /> </div>
        <div className=""> <FaPython className={`text-indigo-900 ${isGlowing ? 'h-[5rem] w-[4rem]' : 'h-[3.5rem] w-[3.5rem]'}`} /> </div>
        <div className=""> <DiRuby className={` ${isGlowing ? 'h-[5rem] w-[4rem]' : 'h-[3.5rem] w-[3.5rem]'}`} /> </div>
        <div className=""><FaNode className={`text-lime-700  ${isGlowing ? 'h-[5rem] w-[4rem]' : 'h-[3.5rem] w-[3.5rem]'}`} />  </div>
        <div className=""><RiBootstrapLine className={`text-violet-800 ${isGlowing ? 'h-[5rem] w-[4rem]' : 'h-[3.5rem] w-[3.5rem]'}`} /> </div>
